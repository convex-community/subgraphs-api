...
def grt_query(endpoint: str, query: str) -> Optional[Mapping[str, List[Mapping[str, Any]]]]:
    for i in range(3):
        ...
        try:
            ...
            logger.error(f"Failed at fulfilling request {query} for {endpoint}, retrying ({i}/3)")
            time.sleep(60)
            continue
    return None


def grt_curve_pools_query(chain: str, query: str) -> Optional[Mapping[str, List[Mapping[str, Any]]]]:
...
def grt_convex_pools_query(query: str) -> Optional[Mapping[str, List[Mapping[str, Any]]]]:
