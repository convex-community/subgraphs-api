...
def get_pool_volume_snapshots(
    ...
    return CurvePoolVolumeSnapshotSchema(many=True).load(
        _exec_query(query), unknown=EXCLUDE
    )
...
def get_pool_fee_snapshots(
    ...
    return CurvePoolFeeSnapshotSchema(many=True).load(
        _exec_query(query), unknown=EXCLUDE
    )
...
def get_pool_tvl_snapshots(
    ...
    return CurvePoolTVLSnapshotSchema(many=True).load(
        _exec_query(query), unknown=EXCLUDE
    )
...
def get_pool_reserves_snapshots(
    ...
    return CurvePoolReserveSchema(many=True).load(
        _exec_query(query), unknown=EXCLUDE
    )
