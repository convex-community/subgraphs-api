from models.convex.snapshot import ConvexPoolSnapshotSchema, ConvexPoolSnapshot, ConvexPoolAPRSnapshot, \
    ConvexPoolAPRSnapshotSchema, ConvexPoolTVLSnapshotSchema, ConvexPoolTVLSnapshot
...
def get_pool_snapshots(pool: str) -> List[ConvexPoolSnapshot]:
    ...
    return ConvexPoolSnapshotSchema(many=True).load(_exec_query(query), unknown=EXCLUDE)
...
def get_pool_tvl_snapshots(pool: str) -> List[ConvexPoolTVLSnapshot]:
    ...
    return ConvexPoolTVLSnapshotSchema(many=True).load(_exec_query(query), unknown=EXCLUDE)
...
def get_pool_apr_snapshots(pool: str) -> List[ConvexPoolAPRSnapshot]:
    ...
    return ConvexPoolAPRSnapshotSchema(many=True).load(_exec_query(query), unknown=EXCLUDE)
